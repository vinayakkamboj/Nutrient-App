  ...
  <body>
    ...
    <div class="upload-screen" id="upload-screen">
      <div class="upload-container">
        ...
        <div class="upload-subtitle">Upload a PDF to get started with the Nutrient Viewer</div>

        <input type="file" id="file-input" accept=".pdf,application/pdf" />
        <button class="upload-button" id="upload-button">Choose PDF File</button>
    ...
    <div class="processing-screen" id="processing-screen">
      ...
      <!-- Details Button -->
      <button class="details-button hidden" id="details-button">Details</button>
    </div>
    ...
    <script>
      // Global function for drawer section toggling
      ...
      (function() {
        ...
        let processingResult = null; // Store processing result for drawer
        ...
        fileInput.addEventListener('change', async function(e) {
          ...
          try {
            ...
            if (result.success && result.url) {
              ...
              uploadStatus.textContent = '✓ PDF uploaded! Processing...';
              uploadStatus.className = 'upload-status';

              // AUTOMATICALLY start processing after 500ms
              setTimeout(async function() {
                try {
                  // Switch to processing screen with spinner
                  ...
                  const processResponse = await fetch(\`${baseURL}/api/process-pdf\`, {
                    ...
                    body: JSON.stringify({
                      pdf_url: pdfUrl,
                      user_prompt: userPromptFromChatGPT
                    })
                  });
                  ...
                  if (processResult.success && processResult.result.response_html) {
                    // Store processing result for drawer
                    processingResult = processResult.result;

                    // Populate drawer with debug info
                    userContent.textContent = processingResult.user_prompt || 'N/A';
                    enhancedContent.textContent = processingResult.enhanced_prompt || 'N/A';
                    responseContent.textContent = processingResult.response_text || 'N/A';

                    // Update metadata
                    const provider = processingResult.provider || 'unknown';
                    const modelRequested = processingResult.model_requested || 'N/A';
                    const modelUsed = processingResult.model_used || 'N/A';
                    ...
                    // Initialize viewer with the generated HTML
                    initializeViewer(processingResult.response_html);
        ...
        function updateDisplay() {
          ...
          if (outputKey !== lastOutput) {
            ...
            // DEBUG: Log when toolOutput changes
            console.log('Tool output updated:', toolOutput);
            if (toolOutput && toolOutput.user_prompt) {
              console.log('USER PROMPT FROM CHATGPT:', toolOutput.user_prompt);
            }

            if (toolOutput && toolOutput.response_html) {
              // We have output! Switch to processing screen
              uploadScreen.classList.add('hidden');
              processingScreen.classList.add('active');
              ...
              if (!viewerInitialized) {
                ...
                // Hide loading overlay after viewer loads
                setTimeout(function() {
                  loadingOverlay.classList.add('hidden');
                  detailsButton.classList.remove('hidden');
                }, 1000);
        ...
        function initializeViewer(responseHtml) {
          try {
            ...
            viewerContainer.innerHTML = '<div class="viewer-empty">Error initializing viewer</div>';
          }
        }
...
`,
      inputSchema: {
        ...
        pdf_url: z.string().optional().describe("Optional PDF URL from uploaded file. If user uploaded a PDF, pass the URL here so it can be used in the generated viewer code."),
      },
      _meta: widgetMeta(globalToolWidget),
    },
    async (args: { user_prompt: string; pdf_url?: string }) => {
      const toolStart = Date.now();
      // IMPORTANT: user_prompt should contain ONLY what the user typed, nothing else
      const { user_prompt, pdf_url } = args;

      try {
        console.log(`[MCP Tool] global_tool (PRIMARY) starting for: "${user_prompt.substring(0, 50)}..."`);

        // ========== CRITICAL: TWO-STAGE FLOW ==========
        // STAGE 1: NO PDF URL → Show upload widget only, DON'T process
        // STAGE 2: PDF URL PROVIDED → NOW process with PromptOrchestrator

        if (!pdf_url) {
          console.log(`[MCP Tool] No PDF URL provided - showing upload widget only (NO processing)`);
          console.log(`[MCP Tool] global_tool completed in ${Date.now() - toolStart}ms (upload stage)`);

          // Return immediately WITHOUT calling PromptOrchestrator
          // Widget will show upload UI, no spinner, no processing
          return {
            content: [
              {
                type: "text",
                text: `📤 Please upload your PDF using the upload button in the widget above.\n\n💡 Once uploaded, provide the PDF URL to process it.`,
              },
            ],
            structuredContent: {
              global_tool: {
                user_prompt,
                enhanced_prompt: "",
                provider: "none" as "anthropic" | "openai",
                model_requested: "N/A",
                model_used: "N/A",
                response_text: "",
                response_html: "", // Empty = widget shows upload screen
                errors: [],
                warnings: [],
                upload_stage: true, // Flag to indicate upload-only stage
              },
        ...
        // PDF URL PROVIDED → NOW start processing
        console.log(`[MCP Tool] PDF URL provided: ${pdf_url}`);
        console.log(`[MCP Tool] Starting PromptOrchestrator (processing stage)...`);

        const orchestratorStart = Date.now();
        const result = await PromptOrchestrator({ user_prompt, pdf_url });
        ...
        return {
          ...
          structuredContent: {
            global_tool: {
              ...
              upload_stage: false, // Processing stage
              pdf_url: pdf_url,
            },
            timestamp: new Date().toISOString(),
          },
